...
def test_find_peaks_filtered(regulome_elasticsearch_client):
    ...
    assert res[0][0]['_source']['coordinates'] == {
        'gte': 70989195, 'lt': 70989345}
    assert (
...
def test_regulome_summary_302(test_client):
    response = test_client.get(
        'summary/?regions=rs10823321&genome=GRCh37&maf=0.01')
...
def test_regulome_summary_tsv(test_client):
    ...
    assert response.get_data().decode(
        'utf-8')[0:163] == 'chrom\tstart\tend\trsids\tprobability\tranking\ttissue_specific_scores\tChIP\tChromatin_accessibility\tFootprint\tFootprint_matched\tIC_matched_max\tIC_max\tPWM\tPWM_matched\tQTL'
    assert response.status_code == 200
