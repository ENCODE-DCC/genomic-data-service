...
class RegionIndexerElasticSearch():
    ...
    def destroy_indices(self):
        if self.es.indices.exists(index=FILES_INDEX):
            self.es.indices.delete(index=FILES_INDEX)

        for assembly in self.assemblies:
            snp_index = SNPS_INDEX_PRE + assembly
            if self.es.indices.exists(index=snp_index):
                self.es.indices.delete(index=snp_index)
            peaks_index_pre = PEAKS_INDEX_PRE + assembly
            for chrom in self.chroms:
                peaks_index = peaks_index_pre + '_' + chrom
                if not self.es.indices.exists(index=peaks_index):
                    self.es.indices.delete(index=peaks_index)

    def setup_residents_index(self):
        if not self.es.indices.exists(index=FILES_INDEX):
            body = self.get_files_index_body()
            print('file index body:')
            print(body)
            self.es.indices.create(index=FILES_INDEX, body=body)

    def setup_snps_index(self):
        for assembly in self.assemblies:
            snp_index = SNPS_INDEX_PRE + assembly
            if not self.es.indices.exists(index=snp_index):
                body = self.get_snps_index_body()
                self.es.indices.create(index=snp_index, body=body)
