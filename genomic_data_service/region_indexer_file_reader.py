...
class S3BedFileRemoteReader:
        ...
        for row in reader:
            ...
            try:
                if self.snp_set:
                    ...
                    (chrom, doc) = self.region(
                        row, value_col=value_col, strand_col=strand_col
                    )
            except Exception:
                logger.error(
                    "%s - failure to parse row %s:%s:%s, skipping row",
                    self.file_properties["s3_uri"],
                    row[0],
                    row[1],
                    row[2],
                )
    ...
    def region(self, row, value_col=None, strand_col=None):
        ...
        if strand_col:
            ...
            elif strand_col - 1 < len(row) and row[strand_col - 1] in [".", "+", "-"]:
    ...
    def snp(self, row):
        ...
        try:
            freq_tag = [tag for tag in info_tags if tag.startswith("FREQ=")][0][5:]
        except IndexError:
            freq_tag = None
        if freq_tag:
