...
def get_rsid_coordinates_from_atlas(atlas, assembly, rsid):
    ...
    log.warning(
        "Could not find %s on %s, using ensemble. Elasticsearch response: %s"
        % (rsid, assembly, snp)
    )
    return None
    ...
    for mapping in mappings:
        if "PATCH" not in mapping["location"]:
            if mapping["assembly_name"] == assembly:
                ...
                return ensembl_assembly_mapper(
                    mapping["location"], species, "GRCh38", assembly
                )
            elif assembly == "GRCm37":
                return ensembl_assembly_mapper(
                    mapping["location"], species, "GRCm38", "NCBIM37"
                )
...
def get_coordinates(query_term, assembly="GRCh37", atlas=None):
    ...
    if query_match:
        ...
        if query_match:
            chrom, start, end = get_rsid_coordinates(
                query_match.group(0), assembly, atlas
            )
        else:
...
def resolve_coordinates_and_variants(region_queries, assembly, atlas, maf):
    ...
    for region_query in region_queries:
        ...
        for snp in snps:
            coord = (snp["chrom"], snp["coordinates"]["gte"], snp["coordinates"]["lt"])
...
def region_get_hits(atlas, assembly, chrom, start, end, peaks_too=False):
    ...
    (peaks, peak_details) = atlas.find_peaks_filtered(
        GENOME_TO_ALIAS[assembly], chrom, start, end, peaks_too
    )
    if not peaks:
    ...
    if peaks_too:
        all_hits[
            "peaks"
        ] = peaks  # For "download_elements", contains 'inner_hits' with positions
    ...
    all_hits[
        "message"
    ] = "%d peaks in %d files belonging to %s datasets in this region" % (
        all_hits["peak_count"],
        all_hits["file_count"],
...
def search_peaks(query_coordinates, atlas, assembly, num_variants):
    ...
    try:
        all_hits = region_get_hits(atlas, assembly, chrom, start, end, peaks_too=True)
        evidence = atlas.regulome_evidence(all_hits["datasets"], chrom, start, end)
    ...
    for peak in all_hits.get("peaks", []):
        documents = [
            resolve_relative_hrefs(document, "document")
            for document in peak["resident_detail"]["dataset"]["documents"]
        ]
        ...
        peak_details.append(
            {
                ...
                "documents": documents,
                "dataset": resolve_relative_hrefs(
                    peak["resident_detail"]["dataset"]["@id"], "dataset"
                ),
                "dataset_rel": peak["resident_detail"]["dataset"]["@id"],
                "biosample_ontology": resolve_relative_hrefs(
                    peak["resident_detail"]["dataset"]["biosample_ontology"],
                    "biosample_ontology",
                ),
            }
        )
...
def score_regions(variants, es, atlas, assembly):
    ...
    for variant in result["variants"]:
        ...
        timings.append({"{}:{}-{}".format(chrom, start, end): (time.time() - begin)})
