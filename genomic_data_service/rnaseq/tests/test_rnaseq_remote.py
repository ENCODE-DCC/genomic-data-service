...
def test_rnaseq_remote_portal_get_dataset_url():
    ...
    assert portal._get_dataset_url() == (
        ...
        '&assay_title=small+RNA-seq&assay_title=polyA+minus+RNA-seq'
        ...
        '&field=biosample_summary&field=replicates.library.biosample.sex'
...
def test_rnaseq_remote_portal_get_file_url():
    ...
    assert portal._get_file_url() == (
        ...
        '&output_category=quantification&file_format=tsv'
        '&assembly=GRCh38&assembly=mm10&assay_title=polyA+plus+RNA-seq'
        '&assay_title=total+RNA-seq&assay_title=small+RNA-seq'
        '&assay_title=polyA+minus+RNA-seq&field=assay_title&field=assembly'
        '&field=biosample_ontology.organ_slims&field=biosample_ontology.term_name'
        '&field=biosample_ontology.synonyms&field=biosample_ontology.name'
        '&field=biosample_ontology.term_id&field=biosample_ontology.classification'
        '&field=dataset&field=donors&field=file_type'
        '&field=genome_annotation&field=href&field=md5sum'
        '&field=output_type&field=s3_uri&field=title'
        '&format=json&limit=all'
    )
