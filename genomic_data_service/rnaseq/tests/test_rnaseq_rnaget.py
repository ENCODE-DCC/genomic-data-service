...
def test_rnaseq_rnaget_mapping_convert_facet_to_filter():
    ...
    facet = {
        ...
        "terms": [{"key": "HLH54F", "doc_count": 3}, {"key": "zfh2", "doc_count": 3}],
        "total": 783,
    }
    expected_filter = {
        "filter": "target.label",
...
@pytest.mark.integration
def test_rnaseq_rnaget_expressions_bytes_view_raise_400_when_filter_not_specified(
    client,
):
    ...
    assert (
        r.json["message"]
        == "400 Bad Request: Must filter by feature (gene) or sample property"
    )


@pytest.mark.integration
def test_rnaseq_rnaget_expressions_bytes_view_raise_400_when_format_not_specified(
    client,
):
...
def test_rnaseq_rnaget_expressions_bytes_tsv_view(client, rnaseq_data_in_elasticsearch):
    ...
    r = client.get("/expressions/bytes?format=tsv&featureNameList=RNF19A")
    actual = list(
        csv.reader(
            StringIO(r.data.decode()),
            delimiter="\t",
...
@pytest.mark.integration
def test_rnaseq_rnaget_expressions_bytes_json_view(
    client, rnaseq_data_in_elasticsearch
):
    from io import StringIO
    import csv

    r = client.get("/expressions/bytes?format=json")
...
@pytest.mark.integration
def test_rnaseq_rnaget_expressions_id_bytes_view_raise_400_when_format_not_specified(
    client,
):
...
@pytest.mark.integration
def test_rnaseq_rnaget_expressions_id_bytes_view_raise_400_when_expression_id_does_not_exist(
    client,
):
...
@pytest.mark.integration
def test_rnaseq_rnaget_expressions_id_bytes_tsv_view(
    client, rnaseq_data_in_elasticsearch
):
    from io import StringIO
    import csv

    r = client.get("/expressions/EXPID002/bytes?format=tsv&searchTerm=(GM23338|tissue)")
    actual = list(
        csv.reader(
            StringIO(r.data.decode()),
            delimiter="\t",
    ...
    assert actual == expected
    r = client.get("/expressions/EXPID001/bytes?format=tsv&file.@id=*")
    actual = list(
        csv.reader(
            StringIO(r.data.decode()),
            delimiter="\t",
...
@pytest.mark.integration
def test_rnaseq_rnaget_expressions_id_bytes_tsv_fpkm_view(
    client, rnaseq_data_in_elasticsearch
):
    from io import StringIO
    import csv

    r = client.get("/expressions/EXPID002/bytes?format=tsv&file.@id=*&units=fpkm")
    actual = list(
        csv.reader(
            StringIO(r.data.decode()),
            delimiter="\t",
...
@pytest.mark.integration
def test_rnaseq_rnaget_expressions_id_bytes_tsv_min_max_view(
    client, rnaseq_data_in_elasticsearch
):
    from io import StringIO
    import csv
...
@pytest.mark.integration
def test_rnaseq_rnaget_expressions_id_bytes_json_view(
    client, rnaseq_data_in_elasticsearch
):
    r = client.get("/expressions/EXPID001/bytes?format=json")
    assert len(r.json["@graph"]) == 4
    r = client.get("/expressions/EXPID001/bytes?format=json&studyID=ENCSR906HEV")
    assert len(r.json["@graph"]) == 4
...
def test_rnaseq_rnaget_expressions_filters_view(client, rnaseq_data_in_elasticsearch):
    ...
    assert len(r.json) == 7
    assert r.json == [
        {"description": "Data Type", "filter": "type", "values": ["RNAExpression"]},
        {
            "description": "Assay title",
            "filter": "file.assay_title",
            "values": ["total RNA-seq", "polyA plus RNA-seq"],
        },
        {
            "description": "Biosample classification",
            "filter": "file.biosample_ontology.classification",
            "values": ["cell line", "tissue"],
        },
        {
            "description": "Biosample term name",
            "filter": "file.biosample_ontology.term_name",
            "values": ["GM23338", "muscle of trunk", "uterus"],
        },
        {"description": "Assembly", "filter": "file.assembly", "values": ["GRCh38"]},
        {
            "description": "Organism",
...
def test_rnaseq_rnaget_service_info(client):
    ...
    assert r.json == {
        ...
        "name": "ENCODE RNAget",
        "organization": {"name": "ENCODE", "url": "https://www.encodeproject.org"},
        "supported": {
            "continuous": False,
            "expressions": True,
            "projects": True,
