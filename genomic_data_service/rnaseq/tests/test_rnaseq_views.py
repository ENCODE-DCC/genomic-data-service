    ...
    assert r.json["facets"] == [
        {
            "field": "type",
            "title": "Data Type",
            "terms": [{"key": "RNAExpression", "doc_count": 16}],
            "total": 16,
            "type": "terms",
            "appended": False,
            "open_on_load": False,
        ...
        },
        {
            ...
            "title": "Organism",
            "terms": [{"key": "Homo sapiens", "doc_count": 16}],
            "total": 16,
            "type": "terms",
            "appended": False,
            "open_on_load": True,
...
@pytest.mark.integration
def test_rnaseq_views_rnaget_search_view_no_results_raises_404(
    ...
    assert len(r.json["@graph"]) == 0
    assert r.status_code == 404
    r = client.get("/rnaget-search/?type=RNAExpression&type=XYZ")
    assert r.status_code == 400
    assert r.json["message"] == "400 Bad Request: Invalid types: ['XYZ']"
...
def test_rnaseq_views_rnaget_report_view(client, rnaseq_data_in_elasticsearch):
    ...
    r = client.get("/rnaget-report/?type=RNAExpression&type=RNAExpression")
    assert r.status_code == 400
    assert (
        r.json["message"]
        == "400 Bad Request: Report view requires specifying a single type: [('type', 'RNAExpression'), ('type', 'RNAExpression')]"
    )
    r = client.get("/rnaget-report/?type=RNAExpression&type=XYZ")
    assert r.status_code == 400
    assert r.json["message"] == "400 Bad Request: Invalid types: ['XYZ']"


@pytest.mark.integration
def test_rnaseq_views_rnaget_search_cached_facets_view(
    client, rnaseq_data_in_elasticsearch
):
    r = client.get("/rnaget-search/?type=RNAExpression")
    assert "@graph" in r.json
...
@pytest.mark.integration
def test_rnaseq_views_rnaget_report_cached_facets_view(
    client, rnaseq_data_in_elasticsearch
):
    r = client.get("/rnaget-report/?type=RNAExpression")
    assert "@graph" in r.json
...
@pytest.mark.integration
def test_rnaseq_views_rnaget_rna_expression_search_generator(
    rnaseq_data_in_elasticsearch,
):
    ...
    path = "/dummy/?type=RNAExpression&searchTerm=RNF19A&field=expression.fpkm&field=gene.symbol"
...
@pytest.mark.integration
def test_rnaseq_views_rnaget_rna_expression_search_generator_manual_request(
    rnaseq_data_in_elasticsearch,
):
    from types import GeneratorType
    from genomic_data_service.rnaseq.searches import rna_expression_search_generator
...
@pytest.mark.integration
def test_rnaseq_views_rnaget_expression_matrix_view(
    client, rnaseq_data_in_elasticsearch
):
